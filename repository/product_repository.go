...
type ProductRepository interface {
	...
	GetNearestProduct(ctx context.Context, tx *gorm.DB, customerCoord string, limit string, offset string, distance string) ([]dto.GetProductResponse, error)
}
...
func (r *productRepository) GetNearestProduct(ctx context.Context, tx *gorm.DB, customerCoord string, limit string, offset string, distance string) ([]dto.GetProductResponse, error){
	if tx == nil {
		tx = r.db
	}
	...
	err := tx.Raw(`SELECT
		...
		ST_DISTANCE(a.coordinates, ?) as "address_distance",
		c.slug,
		c.category_name,
		p.updated_at,
		p.created_at
	...
	ORDER BY
		...
		p.id ASC,
		p.updated_at DESC
	LIMIT ? OFFSET ?;`, customerCoord, customerCoord, distance, customerCoord, limit, offset).Scan(&product).Error
